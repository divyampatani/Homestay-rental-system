...
const Profile = () => {
  ...
  const getUser = async()=> {
    const listingRef =await collection(db, "users");
    const q = query(
      listingRef,
      // where("useRef", "==", auth.currentUser.uid),
      // orderBy("timestamp", "desc")
    ...
    console.log(querySnap);
    let listings = [];
    (await querySnap).docs.filter((subdata)=>subdata.id==auth.currentUser.uid).forEach((doc) => {
      return listings.push({
        id: doc.id,
        data: doc.data().hostimage,
      });
    });
    console.log(listings);
    setuserdata(listings);
